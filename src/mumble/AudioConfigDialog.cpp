...
void AudioInputDialog::on_qpbPushClickPreview_clicked() {
	...
	if (ao) {
		AudioOutputSample *sample = ao->playSample(qlePushClickPathOn->text(), Global::get().s.fCueVolume);
		if (sample)
			...
			ao->playSample(qlePushClickPathOff->text(), Global::get().s.fCueVolume);
	}
}
...
void AudioInputDialog::on_qpbMuteCuePreview_clicked() {
	AudioOutputPtr ao = Global::get().ao;
	if (ao)
		ao->playSample(qleMuteCuePath->text(), Global::get().s.fCueVolume);
}
