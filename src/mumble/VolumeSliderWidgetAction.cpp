...
VolumeSliderWidgetAction::VolumeSliderWidgetAction(QObject *parent)
	: QWidgetAction(parent), m_volumeSlider(createQtUniquePtr(new QSlider(Qt::Horizontal))) {
...
void VolumeSliderWidgetAction::updateTooltip(int value) {
	...
	m_volumeSlider->setToolTip(tooltipText.arg(tr("Volume Adjustment")).arg(value));
}
...
void VolumeSliderWidgetAction::displayTooltip(int value) {
	...
	QToolTip::showText(QCursor::pos(), tooltipText.arg(value), nullptr);
}
